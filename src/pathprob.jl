...
function _pp_forward{T,S}(pp::_PP{T}, P::AbstractMatrix{S})
    ...
    for ((state, observations), prob) in pp
        for (next_state, trans_prob) in column_nz(PT, state)
            _add_if_positive!(next_pp, (next_state, observations),
                              prob*trans_prob)
        end
    end
    next_pp
...
"""
Similar to `_pp_observe`, but sum over the staes. Useful for the last
time period.
"""
function _pp_end{T,S}(pp::_PP{T}, Q::AbstractMatrix{S})
    QT = Q'
    next_pp = Dict{Vector{Int},promote_type(T,S)}()
    for ((state, observations), prob) in pp
        for (observation, trans_prob) in column_nz(QT, state)
            _add_if_positive!(next_pp, vcat(observations, observation),
                              prob*trans_prob)
        end
    end
    next_pp
